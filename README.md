...
| Gateway trace cards were invalid HTML *(broke assertions + a11y)* | Radix `CollapsibleTrigger asChild` injected `type="button"` onto a `<div>`, so 9 trace cards all matched a `div[type=button]` selector | Converted trace cards to real `<button>` elements and added aria-labels to the failure-sim controls |
